...
{
    float z=0+ double (rand())/ RAND_MAX*(1-0);
    return lineA*tan(x)+lineB*cos(x*x)+z;
}
...
void MainWindow::getData()
{
    ...
    lineA = ui->line_a->text().toDouble();
    lineB = ui->line_b->text().toDouble();
    lineC = ui->line_c->text().toDouble();
}
